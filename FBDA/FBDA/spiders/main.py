...
class Spider(object):
    ...
    def parse_detailinfo(self, data):
        data = data.replace(u'\xa9', u'').replace('\\u0022', '"').replace('\\u005C',
                                                                          '\\')  # gbk无法对u'\xa9'代表的字符进行编码，在Unicode中u'\xa9'代表的是©。因此直接忽略掉。
        ...
        result['notPayTotalAmount'] = data['userLoanRecord']['notPayPrincipal'] + data['userLoanRecord'][
            'notPayInterest']  # 待还本息
